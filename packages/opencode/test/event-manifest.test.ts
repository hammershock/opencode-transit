...
describe("public event manifest", () => {
  test("contains every latest public wire type once", () => {
    ...
    expect(EventManifest.Latest.get("session.next.delegation.result.recorded")).toBe(SessionEvent.DelegationResultRecorded)
    expect(EventManifest.Latest.get("session.next.delegation.wake.revoked")).toBe(SessionEvent.DelegationWakeRevoked)
