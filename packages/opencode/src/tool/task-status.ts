...
function statusFailure(cause: Cause.Cause<unknown>) {
...
export function protectStatus<A, E, R>(effect: Effect.Effect<A, E, R>) {
  return effect.pipe(
    Effect.catchCause((cause) =>
      Cause.hasInterruptsOnly(cause) ? Effect.interrupt : Effect.succeed(statusFailure(cause)),
    ),
  )
}
