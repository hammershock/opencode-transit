...
- Functional work is not complete with unit tests alone. Follow [`docs/testing-workflow.md`](docs/testing-workflow.md): Mac build, focused checks, and relevant `opencode-transit` real-workflow acceptance are the default. Add Windows/WSL2 or multiple devices when the change affects their behavior, synchronization, or a documented platform risk; an optional unavailable Windows device does not block a platform-neutral change.
- Build acceptance candidates from a clean, exact PR head or integration commit. For Mac-only work use `bun run script/transit-build.ts --single --skip-install` from `packages/opencode` in an isolated checkout. For required dual-device work use `opencode-transit-dual-build --pr <number-or-url>`; without arguments it builds latest `origin/dev`. Do not use a mutable local `HEAD` as PR acceptance evidence or claim an untested platform passed.
