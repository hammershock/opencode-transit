...
export function render(aliases: readonly string[]) {
  return [
    ...
    "agent_inspect only observes execution state and tool activity. agent_interrupt stops the current execution but keeps the Session available. agent_wait does not interrupt other Agents; a timeout is not task failure. Distinguish admitted, delivered and replied receipts.",
