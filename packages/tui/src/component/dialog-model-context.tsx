...
export function modelContextOptions(generation: ModelContextGeneration, terminalWidth = 100): DialogSelectOption<Preview>[] {
  ...
  const systemParts = new Map((generation.systemParts ?? []).map((part) => [part.key, part]))
  ...
  const modelIdentity = systemParts.get("model")
  if (modelIdentity) {
    options.push({
      category: "SystemPrompt",
      title: "model",
      footer: generation.model ? `${generation.model.providerID}/${generation.model.id}` : undefined,
      value: { title: "model", content: modelIdentity.text },
    })
  }
  ...
  const mcpPart = systemParts.get("mcp")
  if (mcpPart) {
    options.push({
      category: "SystemPrompt",
      title: "mcp",
      footer: mcpPart.tag,
      value: { title: "mcp", content: mcpPart.text },
    })
  }
