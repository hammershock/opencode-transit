...
普通 prompt、TUI 切换、进程重启、冷恢复、compaction 和透明 SSH/Rexd 重连不产生新代际，也不重新读取已接纳规则。rebind 和 `/init` 是用户明确改变工作环境或规则的操作，因此是仅有的运行中 replacement 边界。
...
- TUI 只呈现 `/context` 和 remote operation state，不解析规则或直接访问 target filesystem；
