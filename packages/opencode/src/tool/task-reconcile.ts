...
export const TaskReconcileTool = Tool.define(
  "task_reconcile",
  Effect.gen(function* () {
    ...
    return {
      description: "Explicitly resume or cancel one frozen direct child Task input without changing its identity.",
      ...
      execute: (input: typeof Parameters.Type, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          if (
            ...
            return { title: "Task reconcile unsupported", output: "task_control_unsupported", metadata: {} }
          const execution = Option.getOrUndefined(yield* Effect.serviceOption(SessionExecution.Service))
          if (!execution) return { title: "Task reconcile unavailable", output: "task_unavailable", metadata: {} }
          ...
          if (!ctx.callID)
            return { title: "Invalid Task reconcile", output: "task_reconcile_invalid_request", metadata: {} }
          const receipt = yield* SessionTaskDelivery.reconcile({
            ...
            operationID: `task_reconcile:${ctx.messageID}:${ctx.callID}`,
          ...
          return { title: "Task reconcile receipt", output: JSON.stringify(receipt), metadata: {} }
        }).pipe(
          Effect.catchCause((cause) => {
            if (Cause.hasInterruptsOnly(cause)) return Effect.interrupt
            const error = Cause.squash(cause)
            return Effect.succeed({
              title: "Task reconcile unavailable",
              output:
