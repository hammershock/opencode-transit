...
export function render(
  ...
  return [
    ...
    "Use a clear, stable alias such as /root/review when spawning an Agent. Visible contacts: " +
      ...
      ".",
    "For agent_spawn, use an exact ID from the available subagent catalog when it is shown. If no catalog is shown, general is the built-in type for ordinary work; try that exact ID and let the tool enforce access. Do not guess names such as general-purpose or ask the user to supply a type merely because the catalog is absent. An unavailable type does not start an Agent.",
