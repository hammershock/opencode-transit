...
export function createFetch(override?: FetchHandler, events?: ReturnType<typeof createEventSource>) {
  ...
  const fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (/^\/api\/session\/[^/]+\/(permission|question)$/.test(url.pathname)) return json({ data: [] })
    if (/^\/api\/session\/[^/]+\/activity$/.test(url.pathname)) return json({ activities: [], anchors: [], next: null })
