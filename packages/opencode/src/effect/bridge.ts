...
export interface Shape {
  readonly promise: <A, E, R>(effect: Effect.Effect<A, E, R>, options?: { signal?: AbortSignal }) => Promise<A>
...
export function make(): Effect.Effect<Shape> {
  return Effect.gen(function* () {
    ...
    return {
      promise: <A, E, R>(effect: Effect.Effect<A, E, R>, options?: { signal?: AbortSignal }) =>
        restoreWorkspace(workspace, () =>
          Effect.runPromise(
            // The runtime evaluates synchronously before attaching its abort listener.
            wrap(Effect.suspend(() => (options?.signal?.aborted ? Effect.interrupt : effect))),
            options,
          ),
        ),
